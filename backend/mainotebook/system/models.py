...
from mainotebook.utils.models import CoreModel, table_prefix, get_custom_app_models
...
class Users(CoreModel, AbstractUser):
    ...
    last_failed_login = models.DateTimeField(
        null=True, blank=True,
        verbose_name="最后一次登录失败时间",
        help_text="最后一次登录失败时间"
    )
...
class Dept(CoreModel):
    ...
    @classmethod
    def recursion_all_dept(cls, dept_id, dept_all_list=None, dept_list=None):
        """
        递归获取部门的所有下级部门
    ...
    @classmethod
    def recursion_all_parent_dept(cls, dept_id, dept_list=None):
        """
        递归获取部门的所有上级部门
        :param dept_id: 需要获取的id
        :param dept_list: 递归list
