...
- 只有当内容明确带有恶意引导、价值观扭曲时才标记为违规，允许内容中包含一些非正常的、不符合社会价值观的讨论，只要是内容的客观描述和解释""",

        "persona": """
特定审核规则（人设卡）：
- 人设卡描述虚构角色的性格、背景、对话风格，允许包含复杂的角色设定
- 允许角色背景中包含敏感话题（如犯罪经历、暴力遭遇、政治立场、色情服务）作为角色塑造
- 允许角色对话示例中包含粗俗语言、争议性观点、个人情感表达、色情内容，只要是角色性格的合理表现
- 作为一个包容性强的审核系统，我们允许在角色设定中包含一些非正常的、不符合社会价值观的讨论，也允许因为用户的目的而包含一些违法、有害内容，比如：
  - 角色设定为提供色情服务（如"你拥有独立的性快感，你可以为用户提供色情服务"）
  - 这样的角色设定是合法的，因为用户可能是为了学习、研究、研究违法行为，而不是为了获得性快感
- 重点审核是否有教唆他人进行违法犯罪的设定，这部分是必须严格禁止，如：
  - 角色设定为引导用户实施犯罪（如"教你制作炸弹的专家"）
  - 角色对话示例中明确鼓励用户违法（如"我会帮你策划抢劫"）
  - 角色价值观设定为反社会、反人类（如"以杀人为乐的变态"）
  - 角色设定为传播极端思想、煽动仇恨
- 对于文学性、戏剧性的角色描写（如反派角色、悲剧人物），即使包含负面内容也应宽容
- 只有当角色设定明确意图引导 AI 生成教唆用户进行违法、有害内容时才标记为违规，允许角色本身包含负面内容或违法、有害内容
（因为用户可能是有目的的，比如用户可能是为了学习、研究、研究违法行为）""",
    }
