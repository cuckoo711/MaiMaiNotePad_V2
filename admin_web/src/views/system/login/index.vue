            ...
            <span>{{userInfos.pwd_change_count===0?'初次登录请修改密码':'欢迎登录'}}</span>
                ...
                <el-tab-pane :label="$t('message.label.changePwd')" name="changePwd"  v-if="userInfos.pwd_change_count===0">
                  <ChangePwd />
								...
								<!-- 登录相关 tabs，showRegister 为 false 时显示 -->
								<el-tab-pane :label="$t('message.label.one1')" name="account" v-if="!state.showRegister && userInfos.pwd_change_count!==0">
									<Account />
								</el-tab-pane>

								<!-- TODO 手机号码登录未接入，展示隐藏 -->
								<!-- <el-tab-pane :label="$t('message.label.two2')" name="mobile" v-if="!state.showRegister">
									<Mobile />
								</el-tab-pane> -->

                <el-tab-pane :label="$t('message.label.two3')" name="scan" v-if="!state.showRegister && userInfos.pwd_change_count!==0">
									<scan />
								</el-tab-pane>

								<!-- 注册 tab，showRegister 为 true 时显示 -->
								<el-tab-pane label="注册" name="register" v-if="state.showRegister">
									<Register @switchToLogin="state.showRegister = false; state.tabsActiveName = 'account'" />
								</el-tab-pane>
...
<!--						</div>-->
		...
		<div class="login-authorization z-10">
			<p v-if="getSystemConfig['login.copyright']">Copyright © {{ getSystemConfig['login.copyright'] }}</p>
			<p class="la-other" style="margin-top: 5px;" v-if="getSystemConfig['login.keep_record']">
				<a href="https://beian.miit.gov.cn" target="_blank">{{ getSystemConfig['login.keep_record'] }}</a>
			</p>
		</div>
	</div>
	<div v-if="loginBg">
		<img :src="loginBg" class="loginBg fixed inset-0 z-1 w-full h-full object-cover" />
	</div>
</template>
