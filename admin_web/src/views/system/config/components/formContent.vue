    ...
    <el-form ref="formRef" :model="formData" label-width="0px" label-position="left" style="margin-top: 20px">
      ...
      >
        ...
        <el-col :span="10">
          ...
          <el-input-number :key="`${index}-number`" v-else-if="item.form_item_type_label === 'number'" v-model="formData[item.key]" :min="0"></el-input-number>
          <!--     datetime、date、time     -->
          <el-date-picker
          ...
          <el-tree-select
              :key="`${index}-registration-default-dept`"
              v-else-if="isRegistrationDefaultDept(item)"
              v-model="formData[item.key]"
          ...
          />
          <el-select
              :key="`${index}-registration-default-role`"
              v-else-if="isRegistrationDefaultRole(item)"
              v-model="formData[item.key]"
              placeholder="请选择角色"
              clearable
              filterable
              ...
              collapse-tags
          >
            <el-option v-for="opt in roleOptions" :key="opt.value" :label="opt.label" :value="opt.value"> </el-option>
          </el-select>
          <!--    select      -->
          <el-select
              :key="`${index}-select`"
              ...
              clearable
          >
            <el-option v-for="opt in dictionary(item.setting) || []" :key="opt.value" :label="opt.label" :value="opt.value"> </el-option>
          </el-select>
          <!--    checkbox      -->
          <el-checkbox-group
              :key="`${index}-checkbox`"
              ...
              :placeholder="item.placeholder"
          >
            <el-checkbox v-for="opt in dictionary(item.setting) || []" :key="opt.value" :label="opt.value" :value="opt.value">
              {{ opt.label }}
            </el-checkbox>
          </el-checkbox-group>
          <!--    radio      -->
          <el-radio-group
              ...
              clearable
          >
            <el-radio v-for="opt in dictionary(item.setting) || []" :key="opt.value" :label="opt.value" :value="opt.value">
              {{ opt.label }}
            </el-radio>
          </el-radio-group>
          <!--    switch      -->
          ...
          <!--     图片     -->
          <div v-else-if="['img', 'imgs'].indexOf(item.form_item_type_label) > -1" :key="`${index}-img`">
            <el-upload
                :action="uploadUrl"
          ...
          <!--     文件     -->
          <div v-else-if="['file'].indexOf(item.form_item_type_label) > -1" :key="`${index}-file`">
            <el-upload
                :action="uploadUrl"
                :headers="uploadHeaders"
                name="file"
          ...
          <!--    关联表      -->
          <div v-else-if="['foreignkey', 'manytomany'].indexOf(item.form_item_type_label) > -1" :key="`${index}-table-selector`">
            <table-selector
                v-model="formData[item.key]"
                :el-props="{
								pagination: true,
          ...
          <!--   数组       -->
          <div v-else-if="item.form_item_type_label === 'array'" :key="`${index}-array`">
...
const isRegistrationDefaultDept = (item: any) => {
  const key = String(item?.key || '').trim();
  return props.editableTabsItem?.key === 'registration' && (key === 'default_dept' || key === 'default_department');
};
const isRegistrationDefaultRole = (item: any) => {
  const key = String(item?.key || '').trim();
  return props.editableTabsItem?.key === 'registration' && (key === 'default_role' || key === 'default_roles');
};
// 列表转树形
...
const fetchRoleOptions = async () => {
  try {
    ...
    const rows = res.data || [];
    roleOptions.value = rows.map((r: any) => ({ label: r.name, value: r.id }));
  } catch (e) {
    roleOptions.value = [];
  }
};
...
const getInit = () => {
  api.GetList({ parent: props.options.id, limit: 999 }).then((res: any) => {
    ...
    if (props.editableTabsItem?.key === 'registration') {
      const deptKey = _formData.default_dept !== undefined ? 'default_dept' : _formData.default_department !== undefined ? 'default_department' : '';
      if (deptKey) {
        const deptVal = _formData[deptKey];
        if (deptVal !== undefined && deptVal !== null && deptVal !== '') {
          _formData[deptKey] = typeof deptVal === 'string' ? Number(deptVal) : deptVal;
        }
      }

      const roleKey = _formData.default_roles !== undefined ? 'default_roles' : _formData.default_role !== undefined ? 'default_role' : '';
      if (roleKey) {
        const roleVal = _formData[roleKey];
        if (roleVal === undefined || roleVal === null || roleVal === '') {
          _formData[roleKey] = [];
        } else if (Array.isArray(roleVal)) {
          _formData[roleKey] = roleVal;
        } else if (typeof roleVal === 'string') {
          try {
            const parsed = JSON.parse(roleVal);
            _formData[roleKey] = Array.isArray(parsed) ? parsed : [parsed];
          } catch {
            _formData[roleKey] = roleVal
              .split(',')
              .map((x) => x.trim())
