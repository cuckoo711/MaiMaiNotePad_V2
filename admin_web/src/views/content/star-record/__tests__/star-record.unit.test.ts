...
describe('收藏记录模块单元测试', () => {
	...
	describe('apiPrefix 常量', () => {
		it('应该指向正确的收藏记录端点', () => {
			expect(api.apiPrefix).toBe('/api/content/users/stars/');
		});
	});
	...
	describe('GetObj - 获取收藏记录详情', () => {
		it('应该使用正确的 URL 和 GET 方法', async () => {
			const id = 'star-uuid-123';
			...
			expect(request).toHaveBeenCalledWith({
				url: `/api/content/users/stars/${id}/`,
				method: 'get',
			});
		});
	});

	describe('DelObj - 删除收藏记录', () => {
		it('应该使用正确的 URL 和 DELETE 方法', async () => {
			const id = 'delete-id-456';
	...
	describe('GetStats - 获取收藏统计数据 - 需求 4.6', () => {
		it('应该使用正确的 stats URL 和 GET 方法', async () => {
			vi.mocked(request).mockResolvedValueOnce({
				code: 2000,
				data: { knowledge: 10, persona: 5 },
			});
