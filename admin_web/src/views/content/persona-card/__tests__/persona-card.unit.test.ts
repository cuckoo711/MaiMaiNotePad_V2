...
describe('人设卡模块单元测试', () => {
	...
	describe('API 函数测试', () => {
		...
		describe('UpdateObj - 更新人设卡', () => {
			it('应该使用正确的 URL 和 PUT 方法', async () => {
				...
				expect(request).toHaveBeenCalledWith({
					url: `/api/content/persona/${formData.id}/`,
					method: 'put',
					data: formData,
				});
			});
		...
		describe('exportData - 导出数据', () => {
			it('应该使用正确的导出 URL 和 GET 方法', async () => {
				...
				expect(downloadFile).toHaveBeenCalledWith({
					url: '/api/content/persona/export_data/',
					params: params,
					method: 'get',
				});
			});
		...
		describe('GetFiles - 获取关联文件', () => {
			it('应该使用正确的文件列表 URL 和 GET 方法', async () => {
				...
				expect(request).toHaveBeenCalledWith({
					url: `/api/content/persona/${id}/files/`,
					method: 'get',
				});
			});
		});
	...
	describe('API 端点常量测试', () => {
		it('apiPrefix 应该指向正确的人设卡端点', () => {
			expect(api.apiPrefix).toBe('/api/content/persona/');
		});
	});
});
