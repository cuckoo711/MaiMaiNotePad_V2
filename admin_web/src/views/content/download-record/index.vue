	...
	<fs-page>
		<el-card>
			<fs-crud ref="crudRef" v-bind="crudBinding" @selection-change="handleSelectionChange">
				...
				<template #actionbar-right>
					...
					<!-- 导出 -->
					<el-button
						v-if="auth('download_record:Export')"
						type="primary"
				...
				<template #actionbar-append>
					<el-button
						v-if="auth('download_record:List')"
						type="primary"
						icon="DataAnalysis"
						@click="handleStats"
					>
		...
		>
			<el-table :data="statsList" border stripe empty-text="暂无统计数据">
				...
				<el-table-column prop="count" label="下载次数" align="right" />
			</el-table>
			<template #footer>
...
<script lang="ts" setup name="contentDownloadRecord">
...
const currentRecord = ref<api.DownloadRecord | null>(null);
