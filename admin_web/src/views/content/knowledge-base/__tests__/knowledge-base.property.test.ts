...
describe('知识库模块属性测试', () => {
	...
	describe('属性 2: CRUD 操作数据一致性', () => {
		 ...
		 * 生成符合验证规则的随机知识库数据
		 */
		const knowledgeBaseArbitrary = fc.record({
		...
		it('创建操作：提交后获取的数据应该与提交的数据一致', async () => {
			await fc.assert(
				fc.asyncProperty(knowledgeBaseArbitrary, async (formData) => {
					// 模拟系统生成的字段
					...
					// 模拟创建响应
					const createdData: KnowledgeBase = {
						...formData,
			...
			await fc.assert(
				fc.asyncProperty(
					...
					async (initialData, updateData) => {
						...
						// 模拟更新后的数据
						const updatedData: KnowledgeBase = {
							...existingKnowledgeBase,
							...updateData,
							id: existingId, // ID 不变
							uploader: existingKnowledgeBase.uploader, // 上传者不变
							star_count: existingKnowledgeBase.star_count, // 收藏数不变
							downloads: existingKnowledgeBase.downloads, // 下载次数不变
							create_datetime: existingKnowledgeBase.create_datetime, // 创建时间不变
						...
						expect(fetched.uploader).toBe(existingKnowledgeBase.uploader);
						expect(fetched.create_datetime).toBe(existingKnowledgeBase.create_datetime);

						// 验证更新时间已改变
						expect(fetched.update_datetime).not.toBe(existingKnowledgeBase.update_datetime);
					}
				),
				{ numRuns: 100 }
			);
